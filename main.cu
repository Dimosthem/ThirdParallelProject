...
#define PatchSigma 0.5                          // this is h squared , mentioned in the report
...
__global__ void denoise(float* pixels, float* sigma, int* imageDimension,int* windowDimension,float* means, float* result){
	int xIndex = threadIdx.x+ blockIdx.x* blockDim.x;
	int windowSize;
	int imageSize;
...
int main(void){
    ...
    denoise<<<dimension,dimension>>>(d_image, d_sigma, d_imageDimension, d_windowDimension,d_means, d_finalImage);
    //-------------------------------------
